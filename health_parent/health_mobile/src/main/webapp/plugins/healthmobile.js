...
function getUrlParam(paraName) {
    //  一个只读属性,返回一个 Location 对象,包含有文档的 URL 相关的信息,并提供了改变该 URL 和加载其他 URL 的方法
    var url = document.location.toString();
    //alert(url);
    var arrObj = url.split("?");
    if (arrObj.length > 1) {  //长度大于1,表示地址栏路径包含请求参数
        var arrPara = arrObj[1].split("&");
        var arr;
        for (var i = 0; i < arrPara.length; i++) {
            arr = arrPara[i].split("=");  //若请求参数为id=3&name=jack,那么数组为arr=[id,3],第二次覆盖第一次数组为arr=[name,jack]
